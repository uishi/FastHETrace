...
# Original formula
def ch(ell, beta,  k):
    nlogn = logN *(ell * (beta + 5) + beta + 2*k + 5)
    n     = k*beta*ell + 2*k*beta + 2*k*ell + 4*k + 3*ell + 3
    return nlogn + n
...
def ch_diff_beta_same(ell, beta, k):
    return logN*(beta + 5) + (k*beta + 2*k+3)
